...
centroid_size <- snakemake@input[["centroids"]]
...
```{r blast_rep}
if (snakemake@params[["taxonomy"]] == "sintax"){
	data_table <- read.csv(file = blast_rep, sep = "\t", header= FALSE, check.names=FALSE,
						col.names = c("OTU", "Predicted taxonomy (bootstrap)", "Strand", "Consensus taxonomy","drop")
						) %>% separate("OTU", c("Query", "Count"), sep=";size=")
	data_table$drop <- NULL
} else if (snakemake@params[["taxonomy"]] == "blast"){
	data_table <- read.csv(file = blast_rep, sep = "\t", header= FALSE, check.names=FALSE,
...
for (file in merged_qc){
	data <- read.csv(file = file, sep="\t", check.names=FALSE) %>%
			...
			select(sample, Pos, count, Min_EE, Low_EE, Med_EE, Hi_EE, Max_EE, Mean_Q)
...
# Import centroid sizes
centroid_table <- read.csv(file = centroid_size, sep = "\t", check.names=FALSE)
centroid_table$sample <- as.factor(centroid_table$sample)
```
...
### Merged-read quality distribution

```{r qc_plot qual}

p <- ggplot(qc_table, aes(x=Pos, y=Mean_Q)) +
		geom_line(aes(color=sample)) + 
		labs(x="Position", y= "Mean Phred score") +
		theme_minimal()
p
```

### Merged-read expected error distribution
...
Interactive graph: draw a box to zoom in.

Note that the Sample variable only refers to the origin of the centroid read. A centroid may contain reads from several samples.

```{r centroid}
p <- ggplot(centroid_table, aes(x=size, fill= sample, color=sample)) +
		geom_histogram(binwidth=1, boundary= 0, closed="left") +
		geom_vline(xintercept = snakemake@params[["min_cluster_size"]], linetype="dashed", color = "red") +
...
```{r iframe}
htmltools::tags$iframe(title = "Krona chart", src = "Krona_chart.html", width ="100%", height="800px") 
```

Link to chart: <a href=Krona_chart.html>`r file.path(workdir, snakemake@input[["krona"]])`<a/>
